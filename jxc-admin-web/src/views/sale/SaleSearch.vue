  ...
  <div class="app-container">
    <el-card>
      ...
      <el-table :data="tableData" border stripe v-loading="loading">
        <el-table-column prop="saleNumber" label="单号" width="160" />
        <el-table-column prop="customerName" label="客户" />
        <el-table-column prop="amountPayable" label="应收金额" width="100" />
        <el-table-column prop="amountPaid" label="实收金额" width="100" />
...
import { listCustomer, listSale, detailSale } from '../../api'
...
const loadData = async () => { loading.value = true; try { const res = await listSale(query.value); tableData.value = res.data } finally { loading.value = false } }
const showDetail = async (row) => { const res = await detailSale(row.saleListId); detail.value = res.data; detailVisible.value = true }
