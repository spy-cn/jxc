    ...
    <el-card>
      <div class="search-bar">
        ...
        <el-select v-model="query.state" placeholder="状态" clearable style="width:120px;margin-left:8px">
          <el-option label="已退" :value="1" /><el-option label="未退" :value="2" />
        </el-select>
      ...
      <el-table :data="tableData" border stripe v-loading="loading">
        ...
        <el-table-column prop="amountPayable" label="应退金额" width="100" />
        <el-table-column prop="amountPaid" label="实退金额" width="100" />
        <el-table-column prop="userName" label="操作员" width="100" />
        <el-table-column prop="returnDate" label="日期" width="120" />
        <el-table-column label="状态" width="90"><template #default="{ row }"><el-tag :type="row.state===1?'success':'warning'">{{ row.state===1?'已退':'未退' }}</el-tag></template></el-table-column>
...
import { listSupplier, listReturn, detailReturn } from '../../api'
...
const loadData = async () => { loading.value = true; try { const res = await listReturn(query.value); tableData.value = res.data } finally { loading.value = false } }
const showDetail = async (row) => { const res = await detailReturn(row.returnListId); detail.value = res.data; detailVisible.value = true }
