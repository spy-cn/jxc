  ...
  <div class="app-container">
    <el-card>
      <div class="search-bar">
        <el-select v-model="query.supplierId" placeholder="供应商" clearable style="width:180px" filterable>
      ...
      <el-table :data="tableData" border stripe v-loading="loading">
        <el-table-column prop="purchaseNumber" label="单号" width="160" />
        <el-table-column prop="supplierName" label="供应商" />
        <el-table-column prop="amountPayable" label="应付金额" width="100" />
        <el-table-column prop="amountPaid" label="实付金额" width="100" />
        <el-table-column prop="userName" label="操作员" width="100" />
        <el-table-column prop="purchaseDate" label="日期" width="120" />
        <el-table-column label="状态" width="90"><template #default="{ row }"><el-tag :type="row.state===1?'success':'warning'">{{ row.state===1?'已结算':'未结算' }}</el-tag></template></el-table-column>
    ...
    <el-dialog v-model="detailVisible" title="单据明细" width="700px">
      <el-descriptions :column="2" border>
        <el-descriptions-item label="单号">{{ detail.purchaseNumber }}</el-descriptions-item>
        <el-descriptions-item label="供应商">{{ detail.supplierName }}</el-descriptions-item>
        <el-descriptions-item label="金额">{{ detail.amountPayable }}</el-descriptions-item>
        <el-descriptions-item label="日期">{{ detail.purchaseDate }}</el-descriptions-item>
      </el-descriptions>
      <el-table :data="detail.goodsList" border style="margin-top:12px">
...
import { listSupplier, listPurchase, detailPurchase } from '../../api'
const loading = ref(false); const tableData = ref([]); const suppliers = ref([])
...
const loadData = async () => { loading.value = true; try { const res = await listPurchase(query.value); tableData.value = res.data } finally { loading.value = false } }
const showDetail = async (row) => { const res = await detailPurchase(row.purchaseListId); detail.value = res.data; detailVisible.value = true }
onMounted(async () => { const res = await listSupplier(); suppliers.value = res.data; loadData() })
</script>
