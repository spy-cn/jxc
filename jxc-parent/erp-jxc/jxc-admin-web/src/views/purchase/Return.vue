  ...
  <div class="app-container">
    <el-card>
      <div class="search-bar">
        ...
        <el-button type="primary" :icon="Check" @click="handleSave" style="margin-left:8px" :disabled="!form.supplierId || tableData.length===0">保存退货</el-button>
      </div>
...
import { listSupplier, listGoods, saveReturn } from '../../api'
const suppliers = ref([]); const goodsList = ref([]); const tableData = ref([]); const selected = ref([])
...
const confirmSelect = () => { selected.value.forEach(g => { if (!tableData.value.find(t => t.goodsId === g.goodsId)) tableData.value.push({ goodsId: g.goodsId, goodsCode: g.goodsCode, goodsName: g.goodsName, goodsModel: g.goodsModel, goodsUnit: g.goodsUnit, goodsTypeId: g.goodsTypeId, goodsNum: 1, price: g.purchasingPrice || 0, total: g.purchasingPrice || 0 }) }); dialogVisible.value = false }
const handleSave = async () => { await saveReturn({ supplierId: form.value.supplierId, remarks: form.value.remarks, goodsList: tableData.value }); ElMessage.success('退货成功'); tableData.value = []; form.value = { remarks: '' } }
onMounted(async () => { const res = await listSupplier(); suppliers.value = res.data; loadGoods() })
</script>
