  ...
  <div class="app-container">
    <el-card>
      ...
      <el-table :data="tableData" border stripe v-loading="loading">
        ...
        <el-table-column prop="totalNum" label="采购数量" width="100" />
...
const loadData = async () => {
  loading.value = true
  try { const res = await countPurchase(params.value); tableData.value = res.data; renderChart() } finally { loading.value = false }
}
const renderChart = () => {
  ...
  chart.setOption({
    ...
    xAxis: { type: 'category', data: top.map(d => d.goodsName), axisLabel: { rotate: 30 } },
    yAxis: { type: 'value' },
    series: [{ name: '采购金额', type: 'bar', data: top.map(d => d.totalAmount), itemStyle: { color: '#409EFF' } }]
  })
}
