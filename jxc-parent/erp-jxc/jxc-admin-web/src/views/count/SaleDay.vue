...
const loadData = async () => {
  const res = await countSaleDay(params.value)
  const data = res.data
  if (!chartRef.value) return
  if (!chart) chart = echarts.init(chartRef.value)
  chart.setOption({ title: { text: '按日销售额趋势' }, tooltip: { trigger: 'axis' }, xAxis: { type: 'category', data: data.map(d => d.saleDate) }, yAxis: { type: 'value' }, series: [{ name: '销售额', type: 'line', data: data.map(d => d.totalAmount), smooth: true, areaStyle: {}, itemStyle: { color: '#409EFF' } }] })
}
